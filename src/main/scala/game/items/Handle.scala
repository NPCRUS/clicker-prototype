...
case class DualHandle(mainHand: Weapon,
                      offHand: Weapon) extends Handle {
...
case class OneHandedHandle(mainHand: Weapon,
                           offHand: Option[Shield]) extends Handle {
...
case class TwoHandedHandle(mainHand: Weapon) extends Handle {
  if (!mainHand.twoHanded) throw new IncompatibleHandleException
