...
case class OneTimeActiveEffect(target: EffectTargetType.Type,
                               chance: Double,
                               change: Int,
                               self: Boolean) extends ActiveEffect {
  override def _type: Type = ActiveEffectType.OneTime
}

case class PeriodicActiveEffect(target: EffectTargetType.Type,
                                chance: Double,
                                change: Int,
                                self: Boolean,
                                ticks: Int,
                                tickCd: Int) extends ActiveEffect {
  override def _type: Type = ActiveEffectType.Periodic
}

case class LastingActiveEffect(target: EffectTargetType.Type,
                               chance: Double,
                               change: Int,
                               self: Boolean,
                               duration: Int) extends ActiveEffect {
